...
def transfer_learning_with_plasticity(DNN, BNN, DNN_params, BNN_params:tuple, sigma, batch_size, num_epochs, X_train, X_test, alpha=1, verbose=True):
    '''
    Explores how much DNN must be retrained to approximate a BNN with small plasticity changes.
    ...
    '''
    _Y_train, _Y_test = apply_plasticity_and_generate_new_output(BNN, BNN_params, X_train, X_test, sigma, alpha, verbose=verbose)
    ...
    if isinstance(DNN, RecurrentDNN):
        train_losses, eval_losses = train_rnn(
            ...
            train_loader=train_dataloader, test_loader=test_dataloader,
            optimiser=optimiser, criterion=criterion, num_epochs=num_epochs,
            verbose=True, force_step=False)
    
    return torch.Tensor(train_losses), torch.Tensor(eval_losses)


def plasticity_analysis(DNN, BNN, DNN_params, BNN_params:tuple, sigmas, batch_size, num_epochs, X_train, X_test, verbose):
    '''
    ...
    for i, sigma in enumerate(sigmas):
        ...
        sigma_train_losses, sigma_eval_losses = transfer_learning_with_plasticity(
            DNN, BNN, DNN_params, BNN_params, 
            sigma, batch_size, num_epochs, 
            X_train, X_test, verbose=v)
        
        train_losses_by_sigma[i,:] = sigma_train_losses
        eval_losses_by_sigma[i,:] = sigma_eval_losses
        
    return train_losses_by_sigma, eval_losses_by_sigma
